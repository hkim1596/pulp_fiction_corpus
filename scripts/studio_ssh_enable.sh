...
echo "wait until that window shows the prompt again, then run:   cloudflared tunnel run $TUNNEL"
echo "(the site is unreachable for the few seconds in between). Then continue on the MacBook."
echo "Check afterwards that exactly ONE tunnel process is running:"
echo "   pgrep -fl 'cloudflared tunnel run'"
echo "Two processes = the old one is still alive; connections then fail every other time."
echo "Currently running:"
pgrep -fl "cloudflared tunnel run" | sed 's/^/   /' || echo "   (none)"
